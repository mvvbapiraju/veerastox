...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--visual-only",
        action="store_true",
        help="Only generate visual reports (PNG), skip CSV report files",
    )
    return p.parse_args()
...
def main() -> None:
    ...
    for symbol in symbols:
        ...
        rows.append(row)
        if args.visual_only:
            print(f"backtested {symbol}: {curve_plot_path}")
        else:
            print(f"backtested {symbol}: {curve_path}, {curve_plot_path}")
    ...
    if rows:
        ...
        if args.visual_only:
            ...
            print(f"summary: {summary_path}, {summary_plot_path}")
